...
```php
followDriveItem($item_id): \OpenAPI\Client\Model\DriveItem
```
...
try {
    $result = $apiInstance->followDriveItem($item_id);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling MeDriveApi->followDriveItem: ', $e->getMessage(), PHP_EOL;
}
```
...
try {
    $apiInstance->unfollowDriveItem($item_id);
} catch (Exception $e) {
    echo 'Exception when calling MeDriveApi->unfollowDriveItem: ', $e->getMessage(), PHP_EOL;
}
```
